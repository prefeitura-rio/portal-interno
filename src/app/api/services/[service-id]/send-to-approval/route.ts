...
export async function PATCH(
  ...
  try {
    ...
    console.log('📤 Sending service to approval:', serviceId)
    ...
    // Prepare the service data with status = 0 (draft) and awaiting_approval = true
    ...
    if (response.status === 200) {
      console.log('Service sent to approval successfully:', response.data)
      ...
      return NextResponse.json({
        service,
        success: true,
        message: 'Service sent to approval successfully',
      })
    }

    return NextResponse.json(
      { error: 'Failed to send service to approval' },
